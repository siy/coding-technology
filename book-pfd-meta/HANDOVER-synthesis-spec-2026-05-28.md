...
|---|---|---|
| `architecture` | `(private path)` | Skill complete, verified, dry-runs both workflows |
| `coding-technology/book-pfd` | `book-pfd/` | Synthesis spec v2 written (canon-aligned); Brownfield spec not started; module prose not started |
...
- `HANDOVER-PFD-BOOK.md` was at `coding-technology/` repo root; archived 2026-09-14 to
  `(private path)`. Its durable half is duplicated in
  `(private path)`, which is authoritative.
