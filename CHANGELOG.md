...
### Known gaps
- **The ledger has no `provides / mechanism / costs` entries**, so `next_step`'s press and resolve steps cannot run. `axes-and-ledger.md:39` forward-references them to the reference cards and Card 2 carries only axes and values. The playground reports this as the unexplored-territory halt — "the ledger cannot price this" — rather than guessing containment, and `website/next-step/ledger.js` carries the empty structure ready to be filled. Filling it is authorial work against `axes-and-ledger.md:41-51`: a wrong `provides` entry produces confidently wrong derivations.
