...
The criterion the test turns on is one this book reaches from the process side: you find the change driver because you are already describing what the process does, and the partition falls out of the description. Yannick Loth reaches the same line from the other end and states it as a rule — the [Independent Variation Principle](https://dev.to/yannick555/the-principle-of-independent-variation-as-a-thought-framework-4aaa): unify elements with the same change-driver assignment, separate those with different ones. Two routes to one place, one worn from practice and one cut from first principles, and it is the difference between them that makes the meeting worth noting.

Loth then carries it further than a working method needs to. In [*On the Nature of Cohesion*](https://doi.org/10.5281/zenodo.20785752) he gives cohesion a formal account — correctness relative to a modularization principle, measured along exactly the two axes the test already uses — and the change driver is one such principle. So the practical rule this chapter hands you and the formal result Loth develops are one claim in two registers: the book supplies the articulation from the process side, the paper supplies the grounding underneath it, and neither leans on the other. It is also why the old structural metrics never found cohesion — they measure what code shares, where both accounts measure what governs it.
