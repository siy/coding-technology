...
**This chapter is an experiment, and its design is the point.** Hold the domain fixed and vary only the answer sheet. Selling a seat is selling a seat at every scale, so anything that differs between the three resulting vectors has exactly one possible source. That is the condition Chapter 1's claim requires: if architecture follows from the answers, three sheets over one domain must produce three different vectors, each forced, with no step appealing to taste. The condition cuts both ways, which is what makes it an experiment — if the vectors came out the same, or differed in ways no answer explains, the claim would be in trouble here rather than somewhere safely abstract.

The domain earns its place by spanning the extremes with one product idea. One honesty belongs at the door rather than the exit: these are the book's own systems, graded by their author — home ground proves the mechanics legible, not the method right, and the stricter test follows in the next chapter.
