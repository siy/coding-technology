...
Two of them were pointed at plans, and both are worth knowing. RAND's [Assumption-Based Planning](https://www.cambridge.org/core/books/abs/assumptionbased-planning/contents/1BD0DADB119C345100E51964C5DC7CCA) has asked planners since the late 1980s to name the assumptions a plan leans on, mark the ones that are both load-bearing and likely to break, and give each a signpost: an event that, when it arrives, says the assumption has failed. And in 1972 the executor for [the Shakey robot](http://ai.stanford.edu/~nilsson/OnlinePubs-Nils/PublishedPapers/stripsrevisit.pdf) stored its plan as a table of preconditions, and before each step scanned for the furthest point whose conditions still held: skipping ahead when the world had already done the work, dropping back when a step had failed, replanning when nothing held. The dozen already-finished tickets above are that skip-ahead, half a century later, on a corkboard instead of a robot.

What neither had is a signpost that reads itself. Assumption-Based Planning's signposts are events a human has to watch for, and the robot could simply look at the room. In a codebase, checking a premise by hand is the expensive thing, which is why the discipline needs a cheap mechanical proxy, and why the commit stamp matters: it turns "is this still true?" into a query over history that costs nothing to ask. That, and the fact that nobody has priced the rot, is the whole of the contribution: enumerate the plan's read points, write invalidation when knowledge is created, verify at read time, and stamp what you verified so staleness becomes arithmetic instead of opinion.
