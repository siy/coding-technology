# Knowledge Gathering: Pipeline Data Structures and the mapWith Family
...
The first idea is covered in Chapter 4. This document develops the second one to the same depth:
how to design the data structures that flow between pipeline steps, and how the
context-preserving combinators added in Pragmatica Core 1.0.0-rc1 — `mapWith`, `flatMapWith`,
...
rejection rule. Pipelines are tested per the evolutionary strategy (Chapters 10–11): stub the
...
*Placement suggestion: this material extends the pipeline-composition story of Chapters 7–8 and
presumes Chapter 4 (Parse, Don't Validate). The combinators require Pragmatica Core 1.0.0-rc1+.
The authoritative API reference for the family is the `Result`/`Option`/`Promise` class headers
(`jbct doc Result`); the repo-side pattern reference is
`core/docs/knowledge-gathering-pipelines.md` in the pragmatica repository.*
