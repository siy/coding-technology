...
- **Composition substrate** — synchronous, asynchronous, streaming, or mixed; the cross-subsystem wire that carries typed, versioned facts. *(Phase-5 vector, substrate axis.)*
...
There are eleven questions, in four categories.
...
This is the list the book has been refusing to call "non-functional requirements." That term is defined by what it is not, and it bundles unrelated things — service targets, hard constraints, operating goals, forces that reshape the substrate — under one label that hides the differences. Naming the four categories separately is the whole point. Substrate-shaping in particular earns its own category rather than hiding among constraints: multi-X partitions the data, and scale shape decides what the substrate has to be, before any single target is read.
...
| Composition substrate | synchronous calls / asynchronous events / streaming / mixed |
...
The axes are not fully independent. Some combinations are coherent and some defeat themselves, and the methodology surfaces the incoherent ones as contrast rather than as a chapter of prohibitions. Many deployables wired together with synchronous calls everywhere is a distributed monolith: it pays the latency and operational tax of distribution and keeps none of the autonomy. Serverless functions carrying heavy in-memory state is a mismatch of the topology to the workload. Asynchronous events laced with synchronous inter-service calls at every hop defeats the decoupling the events were chosen for. These are not rules to memorize; they are what the walkthrough will run into and name as it goes.
...
- One deployable, latency budget allows, no cross-unit fan-out → synchronous calls.
- Cross-unit facts that tolerate propagation lag → asynchronous events.
