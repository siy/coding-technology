...
### Added
- **A staleness tripwire relating each course lesson blurb to its book chapter** (`ai-tools/blurb-stamps.py`, `ai-tools/check-drift.sh`) — the 47 blurbs under `website/course/{jbct,pfd,architecture-synthesis}/` shadow book chapters and **no checker read any of them**, so #70 took `book/comparison.md` from 470 lines to 218 while `website/course/jbct/comparison.md` went on describing three deleted sections, with the build's link check, `check-drift.sh` and `sync-book-blocks.py` all green (#69). Each blurb now carries `reviewed-at: <chapter blob id>` in front matter; `check-drift.sh` step 7 fails when a chapter's bytes move without the blurb being re-asserted, and `blurb-stamps.py --update <blurb>` is how an author asserts "I read it and it still holds". The blurb-to-chapter join is filename equality against the book directory, verified in both directions across all 47 with no exceptions — no blurb without a chapter, no course lesson without a blurb.
  **The mechanism is deliberately weak and must stay weak.** A blurb is prose: it cannot be generated from its chapter the way `sync-book-blocks.py` generates a rule block, and comparing the two by meaning is not available. Nothing here reads either text.
  **What a green result does not claim:** anyone can clear the tripwire by running `--update` without reading anything, and the check cannot tell the difference — so a current stamp is an unfalsified assertion, not a verified blurb. Only the failure direction is load-bearing. The same applies to the 47 seeded stamps, which reviewed nothing; they record the chapter state each blurb was already sitting beside so that movement *from* there is caught, and no blurb became verified by acquiring one.
  Verified against the real historical case rather than by a green run: reconstructing the pre-#70 chapter and blurb, stamping, then landing #70's chapter edit alone reddens **exactly 1 of 47** — and the `git cat-file -p <sha> | diff -u -` hint the failure prints surfaces the three deleted `## Traditional Layered` / `## Hexagonal` / `## Clean Architecture` sections. Unstamped blurbs fail, an examined-nothing run fails loudly rather than exiting green, and the count line (`examined 47 blurb(s) against 3 book(s)`) prints on every run so the space checked is never implicit. Blob ids are git's own, computed without invoking git so the check runs with no repository present, which keeps `git log --raw -- <chapter>` usable for locating the last-reviewed revision. Site output is byte-identical with the stamps present (110 files, 89 pages), since `parseCourseLayer` anchors on `## blurb` and never sees front matter.
