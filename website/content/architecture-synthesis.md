...
The first half of the method runs in your browser: the [entry gate](/method/architecture-synthesis/next-step/) checks an answer sheet against the five disciplines — priced, scoped, decomposed, triaged, surfaced — and names what is not yet an answer. It derives no architecture and resolves no judgment point, which is the point: most sheets fail the gate long before a derivation would begin.
