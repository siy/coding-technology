...
- **Composition substrate.** Are use cases invoked through synchronous calls, through asynchronous events, through some hybrid? At one use case the trigger is fixed (synchronous HTTP); the composition question only appears when use cases call other use cases.
