...
private Result<ProcessingSummary> collectCategoryResults(List<Result<CategoryResult>> categoryResults) {
    return Result.allOf(categoryResults)
...
- `Result.allOf` - A custom method that gathers many results into a single result
...
5. **Result Aggregation**: `Result.allOf()` combines multiple Result objects into a single Result.
