...
### Added
- **`next_step` entry gate, as a browser playground** (`website/next-step/`, served at `/method/architecture-synthesis/next-step/`) — the first half of the book's mechanizability claim shipped as software. A dependency-free ES module parses the answer-sheet TOML subset and runs the entry gate's five disciplines (priced · scoped · decomposed · triaged · surfaced), citing the card behind each refusal. It derives no vector and resolves no judgment point; `UNKNOWN` passes the gate as valid input and is never guessed. 25 tests, including tests that pin the *refusals* — that the gate emits no vector and that an UNKNOWN row is neither blocked nor filled in. Runs entirely client-side.
