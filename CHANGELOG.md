...
### Changed
- **Session handovers archived out of the public repo, after the rules they carried were given homes** (`CLAUDE.md`, `website/build.js`, `book-pfd-meta/HANDOVER-synthesis-spec-2026-05-28.md`) — 28 of 30 root `HANDOVER-*.md` files (51,662 words) moved to the private record; the current handover and its immediate predecessor stay, and `CLAUDE.md` now carries the retention rule. The pile's real cost was never reading time: **twenty of the pre-August handovers carried a `Conventions in force` or `Durable decisions` section**, the format switched to additions-only around 2026-08-01, and nothing since restated the full set — so a rule could exist only in a May file, beside a superseded version of itself, with nothing marking which won. Every convention, decision and owner ruling was therefore checked against `CLAUDE.md`, `BOOK-VERSIONING.md`, the project's private authoring specs and project memory before the move. **Almost all of it was already homed**: 15 of 15 owner rulings (the saga ruling is executed verbatim at `book-pfd/architecture-synthesis.md:227`; ArchUnit, RPP naming and the voice ruling in memory; PATCH-vs-major in `BOOK-VERSIONING.md`), and the May-June drafting conventions duplicate the private authoring specs, which `CLAUDE.md` forbids copying into this repo. Genuinely unhomed and now written down: the draft-status rule for `articles/`, "published work is history", private-material-in-records, and the zsh gotchas (`echo ===` fails; `cd` persists across compound commands). Two working-discipline rules went to memory instead.
- **Git history rewritten to purge private material** (all branches and tags, force-pushed 2026-09-23) — removed from every commit: the third-party engagement log, the reviewer's private feedback, the Aether voice overlay, and the 33 archived handover paths; rewritten in every blob: the owner's home-directory paths (repo-relative where the target is this repo, `(private path)` otherwise) and paths into the private repositories. 670 commits became 643, all commit hashes changed, and GitHub's signature badges on old merges are gone. Every difference between the original and rewritten final trees was checked against the removal list and the replacement rules, across all 61 refs; no book, course or site file changed. **Not reached by the rewrite:** GitHub's `refs/pull/*` refs and cached commit views, which need GitHub Support, and forks and clones made before the rewrite.
- **Voice documents are private, all of them** (`book-aether-meta/BOOK-PLAN.md`, three `book-aether-meta` handovers) — the Aether overlay was the last voice document in this repo; its four references now name it as private.
- **The release step no longer says `--admin`** (`CLAUDE.md`) — ruleset 9648960 is scoped to `refs/heads/main` with zero required approvals, so plain `gh pr merge --merge` works; verified by three merges on 2026-09-13. **The installed `/release` skill still says "Branch protection requires admin" and is not tracked in `ai-tools/`** — five of the ten installed skills sit outside the drift net, which is why that line survived.
- **The version declaration splits into `depends_on` and `verified_against`, and the pin moves to `1.0.0-rc3`** (`ai-tools/pragmatica-version.json`, `ai-tools/pragmatica-pins.py`, plus 11 quoting surfaces) — the single `pinned` field was answering two questions, which is why it could never move (#60). `depends_on` is what a reader's build should use: a coordinate, tracking Maven Central mechanically. `verified_against` is what a human actually re-read the API at, and moves only when a person does that work. Bumping the old field re-asserted every verification nobody had performed; holding it still left every snippet six releases stale.
  **`verified_against` is per scope, determined from the repository rather than assumed.** Two values coexisted already — `book/introduction.md:3` at rc1, `book-aether/appendix-a-api-reference.md:4` at rc3 — so a single global value was false for one of them, and they are not a disagreement: the Aether appendix's own text disclaims covering Pragmatica Core, so the two stamps read different API surfaces. Per-*book* also undercounts, because `ai-tools/agents/jbct-{coder,reviewer}.md` carry their own claims beside their own last-modified dates and are not books. Three scopes: `jbct-book` and `jbct-tooling` at rc1, `aether-book` at rc3 with the only real tag/SHA/date stamp in the repository. Keeping the first two apart is a marked judgement, argued from the drift history `check-drift.sh` exists for, not from today's numbers.
  **Every occurrence is assigned to one field or excepted, and the check says which.** Two form rules (a coordinate → `depends_on`; a `release-<version>` ref or a `**Based on:**` header → `verified_against`) cover 18; 9 are declared in `field_assignment` because nothing in the text distinguishes "the build uses rc1" from "somebody read rc1". Anything attributed that neither reaches **fails as unassigned** — never defaulted, because defaulting a verification stamp into `depends_on` fabricates a verification at the next bump. Assignment is per occurrence, not per file: `jbct-reviewer.md` carries both fields, eleven lines apart.
  **21 coordinates moved to `1.0.0-rc3`; 6 verification stamps did not.** Fixed under every reading: `examples/pom.xml:15` pinned core at **0.9.0** while `examples/README.md` beside it claimed rc1. `website/content/aether.md` is now **correct rather than excepted** — `render_exceptions` is empty and the build passes on agreement; its `org.pragmatica-lite.aether:*` wildcard stays unverified and is recorded as an open question. `upstream.known_behind` is deleted, so the scheduled axis now compares Central against `depends_on` and goes green on agreement. `BOOK-VERSIONING.md`'s example generalises to `1.0.0-rcN`: the split makes a single literal there actively misleading, since the books now carry two different library versions for two different reasons.
  **The inventory is re-verified against the split: 19 entries, 15 `historical` unmoved.** `mechanical-deferred` and `editorial` are retired — each now has a real field, so writing one is a bug, and an unknown class fails. One entry is new and was surfaced by the bump alone: `articles/legacy-adoption-strategy.md:76` agreed with the old pin by coincidence, and under the convener's ruling that an article makes a *dated* claim it must not move; classed `dated-publication`, marked as a judgement pending the owner on whether `articles/` is a governed surface.
  **Each field has its own demonstrated failure.** A coordinate mutated to rc2 → `1 findings … depends_on declares 1.0.0-rc3`. A header moved rc1→rc3, the edit a naive bump PR would make → `scope 'jbct-book' was verified at 1.0.0-rc1; moving this literal without a re-read fabricates a verification`. A `historical` line at `0.9.0` does **not** redden while `depends_on` is rc3, and the positive control that its location is reachable is the same line edited to `1.0.0-rc3+`, which fails twice (`1 unassigned`, plus the inventory anchor at `expected 1 occurrence(s), matched 0`). The `pinned` compatibility alias — kept only because `website/build.js` is owned by open PR #75 — fails the check when it disagrees with `depends_on`, probed by setting it to rc1.
- **The declaration-vs-upstream pin axis moved out of PR CI onto a schedule** (`.github/workflows/upstream-pin.yml`, `ai-tools/pragmatica-pins.py`, `ai-tools/check-drift.sh`, `CLAUDE.md`) — of the axes in `pragmatica-pins.py`, exactly one reaches the network: the comparison of the declared pin against Maven Central's `<release>`. Inside a blocking check that axis had only two available behaviours when Central was unreachable, and both were wrong. Failing would block merges on someone else's outage; not failing — what it did — printed `declaration-vs-upstream axis NOT checked` and exited **0** inside an otherwise-green run, which is a gate reporting success while examining nothing. Measured before the change: with `urlopen` raising, `--check` printed that line and returned exit code 0. The axis now runs under a new `--upstream` flag from a daily scheduled workflow (`17 6 * * *`, plus `workflow_dispatch`), where unreachable and malformed are **failures**, not skips — a scheduled job blocks no pull request, so the strictness costs a notification instead of a merge. `--offline` is gone with it; it existed only to suppress a probe `--check` no longer makes.
  **The pointer is verified, not asserted.** A comment saying where an axis went is exactly the kind of claim that rots green after the axis is deleted, so `--check` reads the workflow file on every run and fails if it is missing or has stopped invoking `--upstream` — probed by hiding the file, which turned `--check` red. What did **not** change is what the axis compares: a `known_behind` exception still passes, because it is anchored to one exact upstream value and stops matching the day Central moves. Both failure paths were exercised against the real code, not simulated: an unreachable host (`FAIL: UNREACHABLE … axis NOT checked`, exit 1) and a decayed anchor against live Central (`FAIL: upstream Maven Central publishes 1.0.0-rc3, this repository declares 1.0.0-rc1`, exit 1). The hermetic axes are untouched and still bite: `<version>` mutated to `1.0.0-rc9` in `ai-tools/skills/jbct/SKILL.md` moved the run to `1 findings` and exit 1, and the occurrence counts are otherwise unmoved at `181 total — 38 agree, 27 excepted, 0 findings, 0 need classification, 116 out of scope`.
- **The Pirsch tag points at the first-party proxy** (`website/templates/*.html`, `website/next-step/index.html`, `website/build.js`) — `https://api.pirsch.io/pa.js` replaced with `/p/pl.js` plus explicit `data-hit-endpoint="/p/pv"`, `data-event-endpoint="/p/e"`, `data-session-endpoint="/p/s"`, served by the Cloudflare Worker on `pragmatica.dev/p/*` (worker source and rotation held privately). The driver is measurement bias, not cosmetics: content blockers that blocklist `api.pirsch.io` silently drop hits, and blocker rates differ by channel, so the loss skewed exactly the per-channel comparison the install exists for; first-party paths on our own domain match no blocklist. `data-code` is dropped deliberately — dashboard identity now comes from a per-domain access key held as a Worker secret, selected by the tracked page's hostname, so the tag is domain-neutral and the old cross-domain code hazard no longer exists. `id="pianjs"` must stay exactly: the script locates its own tag by it. `build.js`'s link checker gains a `/p/` exemption — the path exists only at the edge and resolves on no page in `dist/`.
  **Maintenance note (current — replaces the install entry's):** the snippet is still **six independent copies** and the stale-copy hazard is unchanged, but the new tag carries no "pirsch" string, so `grep -rn pirsch website/` no longer finds it. `grep -rn 'id="pianjs"' website/` is the enumeration that works; run it after any change to the script URL or endpoints. Verified after the swap: 6 source copies, 87 of 87 built pages, zero `api.pirsch.io` remnants, live `/p/pl.js` serving the script.
- **The lint-rule counts are removed rather than corrected** (`ai-tools/skills/jbct/SKILL.md`, `ai-tools/skills/aether-coder/SKILL.md`, `ai-tools/agents/jbct-reviewer.md`) — the `jbct` skill claimed 37 rules, `aether-coder` claimed 41, and the CLI ships 69 rule classes. A count in a derived artifact tracking another repository's source rots by construction, so the number is gone from all three rather than updated to a value that would be wrong again at the next release. The `jbct` skill now names the rule-ID families instead (`JBCT-RET-*`, `JBCT-VO-*`, `JBCT-EX-*`, `JBCT-NAM-*`, `JBCT-LAM-*`, `JBCT-STY-*`, `JBCT-LOG-*`, `JBCT-MIX-*`), which are stable and are what every finding is reported under. The CLI has no rule-listing subcommand to point readers at.
- **Q5 attaches at two scopes** (`rules.js`) — load was accepted `per path` only, while `sharded` reads write volume past one node's ceiling and `streaming` reads the one data class whose volume earns a partitioned log, both per data class. A sheet built to the stated rule could not express the fact those values turn on, which is how the Shopify sheet found it. `data-class` is now an accepted Q5 scope, and the book was corrected to match (AS 1.1.2).
- **`separated` follows the rule the derivations actually apply** — read-model divergence plus volume, with staleness as a cost rather than a third condition. The book, the working ledger and the two graded blind runs carried three incompatible statements of this rule while the engine implemented a fourth (divergence alone). `LEDGER.md`, `axes-and-ledger.md:45`, Card 2b and `ledger.js` now agree, and the engine gained the volume condition it lacked plus a cost check that refuses the move where the path is contracted strict.
- **The two values no derivation has ever forced are labelled as such.** `unified runtime` and `serverless` move from "needs a rule" to `UNFORCED`: no corpus run, worked example or blind derivation has produced their forcing pressure, so a rule would not implement the method but invent a demand and then obey it. That is a different gap from a missing rule, and the coverage test now records which kind it is.
- **`read_shape` renamed to `read_model`** across the engine, the four corpus sheets and the spec — *Architecture Synthesis* 1.1.0 named the fact **read-model divergence** and began eliciting it in Card 1's second row, so the field now spells what the book asks for. The old name predated the book having a word for it, and collided with Card 3's demand shapes — the collision the book fix removed. Reverting a single field name breaks four golden assertions, so the wiring is load-bearing rather than decorative.
- **`press.js`'s header no longer claims read-model divergence is unelicited** — Card 1 asks for it as of AS 1.1.0. This is the third stale "does not exist yet" comment found this cycle, after the ledger claim in these notes and `derive.js`'s blocked-press header; each outlived the thing it said was missing. The header now records what the engine genuinely still does not do: `diverges` is an assertion the sheet makes, not a comparison computed across per-unit rows, so Card 5's one-row-per-unit normal form is specified in the book and unenforced in software.

### Known gaps
- **Press coverage is 7 of the ledger's 11 pressable values** (`website/next-step/coverage.test.js`). Unreached: `unified runtime`, `serverless`, `streaming`, `sharded` — each needs a press rule *and* a sheet field to carry the fact the rule would read (coupling and topology uncertainty, duty cycle, per-data-class volume, a natural partition key). The test fails if a ledger value gains a `pressedBy` and nobody decides whether a rule reaches it, so the gap cannot widen silently.
- **`separated` is under-implemented.** `book-arch-meta/LEDGER.md:121` requires three conditions together — a read path with its own tight SLO *and* its own scale shape *and* tolerable staleness. The rule reads shape divergence alone; it reproduced two corpus runs on the strength of those sheets rather than of the rule.
- **Card 2 does not print the ledger.** `axes-and-ledger.md:39` forward-references the per-value `provides / mechanism / costs` entries to the reference cards, and Card 2 carries only axes and values. The content exists — `book-arch-meta/LEDGER.md` v0.2, which `ledger.js` is transcribed from — so this is a publishing gap, not authorial work.
- **Member ordering and import ordering** (`skills/jbct/project-structure/organization.md`) — JBCT-ORD-01 and the import-order rule were in the book (`book/project-structure.md`) but had never reached the skill. Now generated from it.
