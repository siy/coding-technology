...
That is also what keeps the work cheap as the system grows: a piece set by its own edges is never weighed against every piece already on the board, so the effort tracks the number of pieces, not the pairs between them. *Foundations* names the property this buys — *quasi-linear cohesion* — but the jigsaw is the intuition: a sort, not an all-against-all comparison.
...
- **Foundations** names the vocabulary — the shapes, the patterns, the process properties, the telescope, three recovery classes, two axes of cohesion. Read it once; refer back as needed.
